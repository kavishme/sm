...
public class Util {
  ...
  public static byte[] generateGUID() {
    ...
    if ( pool_cnt < 0 ) {
		...
		pool = new byte[pool_size][Volume.RECORD_GUID_SIZE] ; // 32 bytes GUID
		fillPool() ;
    }
  ...
  public static byte[] generateGUID(String key) {
	  return ByteBuffer.allocate(Volume.RECORD_GUID_SIZE).putInt(key.hashCode()).array();
	  }
