 ...
 *          primary interface to all database I/O. SM implementers will provide
 *          the ability to store and fetch arbitrarily size records. Each record
 *          will be indentified by a (hopefully) unique OID. Implementers may
 *          choose to implement one or more OID types and whether or not these
 *          OIDs can be reused.
 */
public interface SM {
	 ...
	 * connect the SM to appropriate disk volumes or one or more files
	 * contianing persistent data. XXX (may consider developping a storage
	 * manager Factory class for instantiating new SM instances and deal with
	 ...
	 *                when there is no record for the specified OID.
	 * @exception IOException
	 *                when there is an IO error in the SM.
	 * @since
	 */
	String fetch(String key) throws NotFoundException, IOException;
	 ...
	 * @since
	 */
	void update(String key, String value) throws NotFoundException, IOException;
	 ...
	 * Implementers may also chose to remove any in memory, cached copies if
	 * that would be appropriate. Implementers may choose whether or not to
	 * reuse the OID for subsequently created records.
	 * 
	 * @param oid
	 *            an OID.
	 * @exception NotFoundException
	 *                when there is no record for the given OID.
	 * @exception CannotDeleteException
	 *                when some other problem occurs.
	 * @since
	 */
	void delete(String key) throws NotFoundException, CannotDeleteException;
	...
	public static class Record {
		...
		public int setBytes(byte[] someData) {
			if (someData == null) {
				...
				int minLength = (data.length < someData.length) ? data.length
						: someData.length;
