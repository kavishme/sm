...
public class SMImplVersion2 implements SM {
	...
	private Hashtable<byte[], byte[]> lookupcache = new Hashtable<byte[], byte[]>(); // useful only for translation table lookups, not durable, initialy updated
	...
	private void trans_table_reset() {		
		...
		while (tbl.position() < (tbl.limit() - Volume.RECORD_GUID_SIZE + Volume.RECORD_ID_SIZE)) {
			...
			lookupcache.put(guid, recid); //fill lookup cache table on initialization to avoid disk I/O
			if (!("00.00.00.00.00.00.00.00".equals(Util.toHexString(recid)))) {
				...
				int newpos = this.translation_table.position() + Volume.RECORD_GUID_SIZE + Volume.RECORD_ID_SIZE; // why use translation_table here, shld be tbl
	...
	private void trans_table_delete_entry(byte[] find_guid) {
		...
		while (tbl.position() < this.translation_table.position()) {
			...
			tbl.get(recid, 0, Volume.RECORD_ID_SIZE);
			if (findstr.equals(Util.toHexString(guid))) {
				int newpos = tbl.position()
						- (Volume.RECORD_GUID_SIZE + Volume.RECORD_ID_SIZE);
				tbl.position(newpos);
				byte zero = 0;
	...
	private void trans_table_dump() {
		...
		while (tbl.position() < (tbl.limit() - Volume.RECORD_GUID_SIZE - Volume.RECORD_ID_SIZE)) {
			...
			tbl.get(recid, 0, Volume.RECORD_ID_SIZE);
			if (!("00.00.00.00.00.00.00.00".equals(Util.toHexString(recid)))) {
				System.out.println("GUID[" + Util.toHexString(guid)
...
//			rec1.setBytes("This is a test #1 abc kavish jain and roommates".getBytes());
...
//				System.out.println(Util.toHexString(found.getBytes(0, 0)));
//				System.out.println(Util.toHexString(rec1.getBytes(0, 0)));
//				System.out.println("");
...
//				//System.out.write(found.getBytes(0, 0));
//				System.out.println("");
//				String rec1hex = Util.toHexString("This is a test #1"
//						.getBytes());
...
//						System.out
//								.println("Failed Stored data is different from retrieved");
//					}
//				}
...
//				System.out.println("Stored Large Record with OID: "
//						+ s_oid.getKey());
//			} catch (Exception e) {
//				System.out.println(e.getMessage());
//			}
...
//					System.out
//							.println("Failed! Stored record is different from excepted one");
//				} else {
...
//				byte[] recid = sm.trans_table_find_entry(oid.toBytes());
//				System.out.println("Stored NULL Record with OID: "
//						+ oid.getKey() + " length: " + rec.length() +  "RECID[" + Util.toHexString(recid) + "] ");
//			} catch (Exception e) {
//				System.out.println(e.getMessage());
//			}
//
...
//		Set<byte[]> keys = lookupcache.keySet();
//		for(byte[] key:keys)
//		{
//			try {
//				Record t = fetch(new OID(key));
//				data.add(t.data.toString());
//			}
//			catch (Exception e) {
...
//	}
	...
	public void store(String key, String value) throws IOException {
		...
		Record therec;
		try {
			therec = new Record(value.length());
			therec.setBytes(value.getBytes());
		} catch (SMException e1) {
			if (DEBUG) {
				System.out.println("Volume Full Error! -- "
						+ e1.getMessage());
				e1.printStackTrace();
			}
		...
		byte[] guid = Util.generateGUID(key);
...
//		}
		...
		// remove null record
		/*
		 * if (rec==null) { if (DEBUG) System.out.println(
		 * "Remove NULL Record with OID: " + Util.toHexString(guid) ) ;
		 * trans_table_delete_entry( guid ) ; try { delete( ret ) ; } catch
		 * (Exception e) {} }
		 */
...
//		}
		...
		// if (DEBUG)
		// System.out.println( "Fetch Using OID: " +
		// Util.toHexString(oid.toBytes()) ) ;
...
//		}
		...
		// if (DEBUG)
		// System.out.println( "Update Using OID: " +
		// Util.toHexString(oid.toBytes()) ) ;
...
//		}
		...
		// if (DEBUG && oid != null)
		// System.out.println( "Delete Using OID: " +
		// Util.toHexString(oid.toBytes()) ) ;
